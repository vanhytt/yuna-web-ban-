...
import { SlidersHorizontal, ShoppingCart, ChevronDown, X } from "lucide-react";
...
import { supabase, getProductImage, normalizeCategorySlug, getCategoryLabel } from "@/lib/supabase";
...
const priceRanges = [
  ...
  { label: "Trên 1.500.000đ", min: 1500000, max: 999999999 },
];
...
export default function CategoryPageClient({ category }: { category: string }) {
      ...
      try {
        ...
        if (saved) {
          try {
            ...
            setProducts([]);
            setError("Lỗi kết nối và dữ liệu sản phẩm cục bộ bị lỗi.");
          }
        } else {
          setProducts([]);
          setError("Lỗi kết nối cơ sở dữ liệu và không có dữ liệu dự phòng.");
        }
      } finally {
        setIsLoading(false);
      }
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (currentCategory !== "san-pham" && currentCategory !== "products") {
      list = list.filter((p) =>
            ...
            {isLoading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {Array.from({ length: 6 }).map((_, idx) => (
                  <div
                    key={idx}
                    className="bg-white rounded-xl border border-gray-100 overflow-hidden p-4 flex flex-col gap-3"
                  >
                    <div className="relative aspect-square w-full rounded-lg bg-gray-200 overflow-hidden animate-shimmer" />
                    ...
                    <div className="h-6 w-24 bg-gray-200 rounded animate-shimmer mt-1" />
                    <div className="h-10 w-full bg-gray-200 rounded-lg animate-shimmer mt-2" />
                  </div>
                ))}
              </div>
            ) : error ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                <p className="text-red-500 text-base mb-1 font-semibold">{error}</p>
                <button
                  onClick={() => window.location.reload()}
                  className="text-[#C59B27] font-semibold hover:underline mt-2 cursor-pointer"
                >
                  Tải lại trang
                </button>
              </div>
